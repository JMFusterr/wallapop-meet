...
- El banner superior de venta pendiente en chat usa color de marca Wallapop (verde) en fondo y accion.
...
- Reglas actuales del paso 2 (selector de ubicacion):
...
- Mapa en selector de punto:
  ...
  - Al seleccionar punto personalizado se muestra direccion (reverse geocoding con fallback) y distancia.
...
- En `ARRIVED`, la accion `Confirmar venta` usa color de vendido (`sold`, rosa), no color de reservado (`reserve`, morado).
...
- Si el componente necesario no existe:
  ...
  - Verificar que el estado/caso nuevo quede visible tambien en el Design System vivo (`src/pages/design-system-page.tsx`), directamente o a traves de la story.
